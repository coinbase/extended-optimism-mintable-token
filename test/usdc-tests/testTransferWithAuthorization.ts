...
import { ACCOUNTS_AND_KEYS, MAX_UINT256 } from "./helperUtils/constants";
import { expectRevert, hexStringFromBuffer } from "./helperUtils";
import {
...
export function testTransferWithAuthorization({
  ...
  describe("transferWithAuthorization", () => {
    ...
    it("reverts if the authorization is not yet valid", async () => {
      ...
      // create a signed authorization that won't be valid until 100 seconds
      // later
    ...
    it("reverts if the payer, payee or the sender is blacklisted", async () => {
        const { from, to, value, validAfter, validBefore } = transferParams;
        // create a signed authorization
        const { v, r, s } = signTransferAuthorization(
          from,
